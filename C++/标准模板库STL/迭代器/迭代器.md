...
## 为什么会有迭代器
在进行对某个算法操作能够最大限度的处理不同类型的容器和数据元素时，我们尝试使用指针来处理操作，这种方法对array和vector的效果是很好的，但是放到list中，就会不兼容，需要做一些更改，那么“万能法则”：解决计算机的问题可以通过增加层来实现。所以我们的解决办法是在底层指针的行为之上提供一层抽象，也就是我们要学习的迭代器。
## 了解Iterator
