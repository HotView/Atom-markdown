#### 试除法判定质数(素数) —— 模板题 AcWing 866. 试除法判定质数
```
bool is_prime(int x)
{
    if (x < 2) return false;
...
```
#### 试除法分解质因数（约数：分母） —— 模板题 AcWing 867. 分解质因数
```c
...
#### 试除法求所有约数（因数：分母） —— 模板题 AcWing 869. 试除法求约数
```c
